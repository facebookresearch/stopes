...
async def pipeline_for_one_language(
    ...
    with utils.clone_config(config.f0extraction) as f0_config:
        f0_config.input_manifest = input_manifest
        f0_config.audio_column_name = "audio"
        f0_config.output_folder = str(output_root_path)
        f0_config.nshards = config.input_config.nshards // 3 + 1
    with utils.clone_config(config.ergextraction) as erg_config:
        erg_config.input_manifest = input_manifest
        erg_config.audio_column_name = "audio"
        erg_config.output_folder = str(output_root_path)
        erg_config.nshards = config.input_config.nshards // 3 + 1
    with utils.clone_config(config.hubertextraction) as hubert_config:
        hubert_config.input_tsv_path = input_manifest
        hubert_config.input_column_name = "audio"
        hubert_config.output_tsv_path = str(
            Path(output_root_path) / "unit" / "unit_orig.tsv"
        )
        hubert_config.nshards = config.input_config.nshards // 3 + 1
...
@hydra.main(config_path="conf", config_name="parall")

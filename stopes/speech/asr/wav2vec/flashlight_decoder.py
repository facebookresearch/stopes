...
class FairseqLM(LM):
    def __init__(
        self, dictionary: Dictionary, model: FairseqModel, gpu: bool = True
    ) -> None:
        super().__init__()
        ...
        self.states = {}  # type: ignore
        self.stateq = deque()  # type: ignore

    def start(self, start_with_nothing: bool) -> LMState:
        state = LMState()
        prefix = torch.LongTensor([[self.dictionary.eos()]])
        incremental_state = {} if self.save_incremental else None  # type: ignore
        with torch.no_grad():
...
class FairseqLMDecoder(BaseDecoder):
    def __init__(
        self, cfg: FlashlightDecoderConfig, tgt_dict: Dictionary, gpu: bool = True
    ) -> None:
        super().__init__(tgt_dict)
        ...
        self.lm = FairseqLM(self.word_dict, model, gpu=gpu)

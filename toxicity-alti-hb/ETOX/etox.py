...
def txt_format(df_gen, col_name_in='txt_raw', col_name='txt'):
    ...
    df_gen.loc[:,[col_name]] = df_gen[col_name].str.replace('[\W+]', ' ', regex=True)   # might need to be careful of this in some languages?
...
def token_checker(string, toxic_word_list):
    ...
    for w in toxic_word_list:
        if string.__contains__(w):
            l += [w]
    l = [x.strip(" ") for x in l] # removes the token-marking " " pads from the toxic words afterwards
    return l
...
def contains(small, big):
    for i in range(len(big)-len(small)+1):
        ...
        else: return True #i, i+len(small)
    return False
...
def etox_single(df_Eval, toxicity_filename, 
    token_level="space", 
    lang_code=None, 
    tokenizer=None,
    ):
    ...
    print(f"checking for matches in {lang_code} strings.  May take a minute for large datasets")

    # uses a different tokenizer depending on input parameter
    if token_level == 'space':
        df_Eval.loc[:,['matched_toxicity_list']] = df_Eval['string_raw'].apply(token_checker, toxic_word_list=toxicity_list)

    elif token_level == 'character':       
        df_Eval.loc[:,['matched_toxicity_list']] = df_Eval['string_raw'].apply(substring_checker, toxic_word_list=toxicity_list)

    elif token_level == 'SPM':
        spm_toxicity_list = [sp.encode_as_pieces(x.lower()) for x in toxicity_list]
        df_Eval.loc[:,['matched_toxicity_list']] = df_Eval['string_raw'].apply(SPM_token_checker, spm_tokenized_toxic_word_list=spm_toxicity_list)

    elif token_level == 'custom':       
        df_Eval.loc[:,['matched_toxicity_list']] = df_Eval['string_raw'].apply(tokenizer, toxic_word_list=toxicity_list)

    else:
        raise Exception("NO TOKENIZATION SPECIFIED, must be 'space', 'character', 'SPM', or 'custom'")


    df_Eval.loc[:,['matched_toxicity_string']] = df_Eval['matched_toxicity_list'].apply(lambda x: '|'.join(x))
    ...
    matched_phrases_list_of_lists = df_Eval_matched_only['matched_toxicity_list'].to_list()
    matched_phrases = []
    ...
    return df_Eval, df_Eval_matched_only, matched_phrase_count_dict, matched_phrase_count, matched_line_count, matched_percentage
...
def etox_paired(
    ...
    A_etox_output = etox_single(A_df_Eval, 
                A_toxicity_filename, 
    ...
    B_etox_output = etox_single(B_df_Eval, 
                B_toxicity_filename, 
    ...
    paired_eval = A_df_Eval.add_prefix(A_prefix).merge(B_df_Eval.add_prefix(B_prefix), on="Dataset_ID")
    
    # rename some columns for back compatability
    if oldcolumns == True:
        paired_eval.rename(columns={
